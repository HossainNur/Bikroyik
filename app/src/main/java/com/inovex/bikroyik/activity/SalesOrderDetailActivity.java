...
import static com.inovex.bikroyik.AppUtils.AppDatabaseHelper.COLUMN_ORDER_PRICE;
import static com.inovex.bikroyik.AppUtils.AppDatabaseHelper.COLUMN_ORDER_PRODUCT_ID;
import static com.inovex.bikroyik.AppUtils.AppDatabaseHelper.COLUMN_ORDER_PRODUCT_QUANTITY;
import static com.inovex.bikroyik.AppUtils.AppDatabaseHelper.COLUMN_ORDER_PRODUCT_TITLE;
...
public class SalesOrderDetailActivity extends AppCompatActivity {
    ...
    ArrayList<HashMap<String, String>> retailerInfoList = new ArrayList<>();
    ...
    String orderRetailerIdFromIntent = "";
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_update_sales_order);
        ...
        tvUpdateOrder = findViewById(R.id.tvUpdateOrder);
        ...
        //  retailerNameList = getRetailerNameList(retailerList);
        ...
        if (intent != null) {
            orderIdFromIntent = intent.getExtras().getString("order_id");
            orderRetailerIdFromIntent = intent.getExtras().getString("retailer_id");
            // Toast.makeText(mContext,"order Id: "+orderIdFromIntent,Toast.LENGTH_SHORT).show();


            Log.v("_sf", " order id: " + orderIdFromIntent + " retailer id: " + orderRetailerIdFromIntent);
            // get retailer info from db with id

            //retailerInfoList = appDatabaseHelper.getRetailerInfo(orderRetailerIdFromIntent);

            retailerId = retailerInfoList.get(0).get(AppDatabaseHelper.COLUMN_RETAILER_ID);
            retailerName = retailerInfoList.get(0).get(AppDatabaseHelper.COLUMN_RETAILER_NAME);
            retailerAddress = retailerInfoList.get(0).get(AppDatabaseHelper.COLUMN_RETAILER_ADDRESS);
            distributorName = retailerInfoList.get(0).get(AppDatabaseHelper.COLUMN_RETAILER_DISTRIBUTOR_NAME);
            contactPhone = retailerInfoList.get(0).get(AppDatabaseHelper.COLUMN_RETAILER_PHONE);
            retailerLatitude = retailerInfoList.get(0).get(AppDatabaseHelper.COLUMN_RETAILER_LATITUDE);
            retailerLongitude = retailerInfoList.get(0).get(AppDatabaseHelper.COLUMN_RETAILER_LONGITUDE);
            retailerNameList = getRetailerNameList(retailerInfoList);
            spinner.setItems(retailerNameList);
            spinner.setSelectedIndex(1);
            spinner.setEnabled(false);


            // get all ordered product from order_product table
            orderInfoList = appDatabaseHelper.getOrderInfo(orderIdFromIntent);
            orderProductList = appDatabaseHelper.getOrderProductList(orderIdFromIntent);
            Log.v("_sf", "orderInfoList  size :" + orderInfoList.size());
            Log.v("_sf", "orderProductList  size :" + orderProductList.size());
            newOrderList = populateOrderList(orderProductList);
        ...
        tvClearList.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                //      code to  clear the whole list and list data
                //     newOrderList.clear();
                //     newOrderRecyclerAdapter.notifyDataSetChanged();

                // code to clear the last row in the list
                if (newOrderList.size() > 0) {
                    newOrderList.remove(newOrderList.size() - 1);
                    newOrderRecyclerAdapter.notifyDataSetChanged();
                    updateTotalPriceCalculation();
                } else {
                    // nothing to do
        ...
        // spinner.setItems("Choose Retailer Point ", "Bangladesh pharmacy", "Cure drug house", "medica drug corner", "Trust drug house", "North medicine house", "Heaven drug pharmacy");


//        // commented for demo data
//        spinner.setItems(retailerNameList);
//
...
//                // commented for demo
//                if (position > 0) {
//                    retailerId = retailerList.get(position - 1).get(AppDatabaseHelper.COLUMN_RETAILER_ID);
//                    retailerName = retailerList.get(position - 1).get(AppDatabaseHelper.COLUMN_RETAILER_NAME);
//                    retailerAddress = retailerList.get(position - 1).get(AppDatabaseHelper.COLUMN_RETAILER_ADDRESS);
//                    distributorName = retailerList.get(position - 1).get(AppDatabaseHelper.COLUMN_RETAILER_DISTRIBUTOR_NAME);
//                    contactPhone = retailerList.get(position - 1).get(AppDatabaseHelper.COLUMN_RETAILER_PHONE);
//                    retailerLatitude = retailerList.get(position - 1).get(AppDatabaseHelper.COLUMN_RETAILER_LATITUDE);
//                    retailerLongitude = retailerList.get(position - 1).get(AppDatabaseHelper.COLUMN_RETAILER_LONGITUDE);
//
//                } else {
//                    retailerId = null;
//                    retailerName = null;
...
//        });
        ...
        tvUpdateOrder.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ...
                if (retailerId != null && retailerName != null) {
                    ...
                    if (newOrderList.size() > 0) {
                        final String srId = "5809631796";
                        final double total = getTotalOrderPrice();
                        final String discount = "5";
                        final double grandTotal = total - (total * 0.05);
                        ...
                        final String orderId = orderIdFromIntent;
                        ...
                        if (AppUtil.isNetworkAvailable(getApplicationContext())) {
                            ...
                            updateOrderInLocalDatabase(orderId, orderDate, orderTime, orderStatusOffline, total + "", discount, grandTotal + "", srId, retailerId, retailerName, retailerAddress, contactPhone, distributorName);
                        }
                    } else {
    ...
    private List<NewOrderItem> populateOrderList(ArrayList<HashMap<String, String>> orderProductList) {


        Log.v("_sf", "product list size :" + orderProductList.size());
        ArrayList<HashMap<String, String>> orderProductsList = orderProductList;
        List<NewOrderItem> newOrderItemList = new ArrayList<>();

        for (int x = 0; x < orderProductsList.size(); x++) {

            HashMap<String, String> orderProductMap = orderProductsList.get(x);
            String productId = orderProductMap.get(COLUMN_ORDER_PRODUCT_ID);
            String productName = orderProductMap.get(COLUMN_ORDER_PRODUCT_TITLE);
            int productQuantity = Integer.parseInt(orderProductMap.get(COLUMN_ORDER_PRODUCT_QUANTITY));
            double totalPrice = Double.parseDouble(orderProductMap.get(COLUMN_ORDER_PRICE));
            ...
            newOrderItem.setQuantity(productQuantity);
            newOrderItem.setProductPositionInList(x + 1);
    ...
    private void updateOrderInLocalDatabase(String orderId, String orderDate, String orderTime, String orderStatus, String total, String discount,
        ...
        appDatabaseHelper.updateOrder(orderId, orderDate, orderTime, orderStatus, total, discount, grandTotal, orderDate, srId, retailerId, retailerName, retailerAddress, contactPhone, distributorName);

        appDatabaseHelper.deleteSingleOrderProducts(orderId);
        // delete all product with the order_id
        for (int x = 0; x < newOrderList.size(); x++) {
            NewOrderItem newOrderItem = newOrderList.get(x);
            Log.v("_sf", "sales ord actvt saveOrderInLocalDatabase product price: " + newOrderItem.getTotalPrice());

            // and insert all product
            ...
            StringRequest stringRequest = new StringRequest(Request.Method.POST, URL, new Response.Listener<String>() {
                ...
                public void onResponse(String response) {
                    ...
                    if (response.toString().contains("success")) {
                        ...
                        updateOrderInLocalDatabase(orderId, orderDate, orderTime, orderStatus, total + "", discount, grandTotal, srId, retailerId, retailerName, retailerAddress, contactPhone, distributorName);
    ...
    private void updateTotalPriceCalculation() {
        ...
        double grandPrice = totalPrice - (totalPrice * 0.05);
