...
import com.inovex.bikroyik.data.model.RentModel;
...
public class RentActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ProgressDialog csprogress=new ProgressDialog(RentActivity.this);
        ...
        saveButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (!TextUtils.isEmpty(expenseAmount.getText().toString())){
                    rentModel.setAmount(Integer.parseInt(expenseAmount.getText().toString()));
                }else {
                    flag = true;
                ...
                if (!TextUtils.isEmpty(expenseNote.getText().toString())){
                    rentModel.setNote(expenseNote.getText().toString());
                }else {
                    flag = true;
                ...
                if (!flag){
                    ...
                    rentModel.setExpenseType("Rent");

                    if (imageName !=null && currentPhotoPath != null){
                        rentModel.setImage(imageName);
                    }
                    if (!TextUtils.isEmpty(dateButton.getText().toString())){
                        rentModel.setDate(dateButton.getText().toString());
                    }else {
                        rentModel.setDate("");
                    }


                    rentModel.setStoreId(sharedPreference.getStoreId());
                    rentModel.setSubscriberId(sharedPreference.getSubscriberId());
                    rentModel.setSubmittedBy(sharedPreference.getUserId());


                    sendClientInfoToServer(getApplicationContext(), ApiConstants.EXPENSE, rentModel);
        ...
        expenseImage.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                PopupMenu popupMenu = new PopupMenu(RentActivity.this, expenseImage);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == CAMERA_REQUEST && resultCode == Activity.RESULT_OK)
        {
            ...
            try {
                ...
                e.printStackTrace();
                Toast.makeText(RentActivity.this, "Something went wrong", Toast.LENGTH_LONG).show();
            }
        }
    ...
    private String prepareJson(RentModel rentModel){
        Gson gson = new GsonBuilder().serializeNulls().create();

        return gson.toJson(rentModel);
    }
...
//            }
    ...
    private void sendClientInfoToServer(Context context, String url, RentModel rentModel){
        VolleyMethods volleyMethods = new VolleyMethods();
        String reqBody = prepareJson(rentModel);
        Log.d("_tag_", "json: "+reqBody);

        volleyMethods.sendPostRequestToServer(context, url, prepareJson(rentModel), new VolleyCallBack() {
            @Override
            public void onSuccess(String result) {
                ...
                if (!TextUtils.isEmpty(result) && "error".equals(result)){
                    Log.d("_tag_", "response: "+result);
                    Toast.makeText(RentActivity.this,"Expense added Successfully",Toast.LENGTH_SHORT).show();
                    Intent intent = new Intent(RentActivity.this, Expense.class);
                    startActivity(intent);

                }
                else {
                    Toast.makeText(RentActivity.this,"Server Error",Toast.LENGTH_SHORT).show();
