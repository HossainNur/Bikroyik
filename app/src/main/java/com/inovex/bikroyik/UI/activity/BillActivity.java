...
public class BillActivity extends AppCompatActivity {
    ...
    private TextView expense_amount,expense_note,toolbar_title,expenseAmountText,expenseNoteText,dateText,buttonText;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ProgressDialog csprogress=new ProgressDialog(BillActivity.this);
        ...
        saveButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (!TextUtils.isEmpty(expenseAmount.getText().toString())){
                    billModel.setAmount(Integer.parseInt(expenseAmount.getText().toString()));
                }else {
                    flag = true;
                ...
                if (!TextUtils.isEmpty(expenseNote.getText().toString())){
                    billModel.setNote(expenseNote.getText().toString());
                }else {
                    flag = true;
                ...
                if (!flag){
                    ...
                    billModel.setExpenseType("Bill");
                    if (imageName !=null && currentPhotoPath != null){
                        billModel.setImage(imageName);
                    }
                    if (!TextUtils.isEmpty(dateButton.getText().toString())){
                        billModel.setDate(dateButton.getText().toString());
                    }else {
                        billModel.setDate("");
                    }


                    billModel.setStoreId(sharedPreference.getStoreId());
                    billModel.setSubscriberId(sharedPreference.getSubscriberId());
                    billModel.setSubmittedBy(sharedPreference.getUserId());



                    sendClientInfoToServer(getApplicationContext(), ApiConstants.EXPENSE, billModel);
        ...
        expenseImage.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                PopupMenu popupMenu = new PopupMenu(BillActivity.this, expenseImage);
    ...
    private String prepareJson(BillModel billModel){
...
//            }
    ...
    private void sendClientInfoToServer(Context context, String url, BillModel billModel){
        VolleyMethods volleyMethods = new VolleyMethods();
        String reqBody = prepareJson(billModel);
        Log.d("_tag_", "json: "+reqBody);

        volleyMethods.sendPostRequestToServer(context, url, prepareJson(billModel), new VolleyCallBack() {
            @Override
            public void onSuccess(String result) {
                ...
                if (!TextUtils.isEmpty(result) && "error".equals(result)){
                    Log.d("_tag_", "response: "+result);
                    Toasty.success(BillActivity.this, "Expense added Successfully!", Toast.LENGTH_SHORT).show();
                    Intent intent = new Intent(BillActivity.this, Expense.class);
                    startActivity(intent);
                }
                else{
                    /*Toast toast=Toast.makeText(BillActivity.this,result.toString(),Toast.LENGTH_SHORT);
                    toast.show();*/
                    Toast toast=Toasty.error(BillActivity.this, result.toString(),Toast.LENGTH_SHORT);
                    toast.show();
                }
